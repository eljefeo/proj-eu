...
import ProjEuProblems.Problem82Helper;
...
public class Util {
	...
	public static int lcm(int a, int b) { //assuming both nums are positive
		int h, l;
		if(a < b) {
			l = a;
			h = b;
	...
	public static long lcm(long a, long b) { //assuming both nums are positive
		long h, l;
		if(a < b) {
			l = a;
			h = b;
	...
	public static long[] lcmWithMult(long a, long b) { //assuming both nums are positive
		...
		// the second thing in the array is how much to multiply the HIGHER number to to get the lcm
...
//			1/8, 1/7, 1/6, 1/5, 1/4, 2/7, 1/3, 3/8, 2/5, 3/7, 1/2, 4/7, 3/5, 5/8, 2/3, 5/7, 3/4, 4/5, 5/6, 6/7, 7/8
	...
	public static int howManyWaysToSumToNEulers_divisibleByN(int numDivisible) {
		...
		while(true) {
			...
			run %= numDivisible; // we can store only the last 6 digits because 
				// if we add or multiply these together, the numbers left of the first N digits don't matter or have any 
				// effect on future adding/multiplication. We only care if the first N digits are all zeros
			if(run == 0) {
				return ii;
			}
			all.add(run);
	...
	/*
	public static int howManyWaysToSumToNEulers_divisibleByN(int n) {
		List<BigInteger> all = new ArrayList<BigInteger>();
		all.add(new BigInteger("1"));
		all.add(new BigInteger("1"));
	
		BigInteger million = new BigInteger("" + n);
		int ii = 2;
		while(true) {
